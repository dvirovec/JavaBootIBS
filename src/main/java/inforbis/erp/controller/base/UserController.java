...
public class UserController {
    ...
    @RequestMapping(value="erp/login/{username}/{password}/{client}", method= RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE)
        public ResponseEntity<User> login(@PathVariable("username") String username, @PathVariable("password") String password, @PathVariable("client") String client_name, HttpSession session) {

            HttpStatus status = HttpStatus.OK;

            securtityService.autologin(username,password);

            Iterable<Client> clients = clientService.findClientByName(client_name);
